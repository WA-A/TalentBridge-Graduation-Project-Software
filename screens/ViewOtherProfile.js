...
export default function ViewOtherProfile ({ navigation,route}) {
    const { userData } = route.params || {}; // استلام بيانات المستخدم
  ...
  const handleViewProfile = async () => {  
      ...
      seUserName(userData.UserName);
      setAbout(userData.About);
      setBio(userData.Bio);
      setLocation(userData.Location);
      setProfileimage(userData.PictureProfile?.secure_url || '');
      setCoverimage(userData.CoverImage?.secure_url || '');
      setnewProfileImage(profileImage);
      setnewCoverImage(CoverImage);
      setnewAbout(about);
      setnewBio(bio);
 ...
 //     socket.emit('profileUpdated', userData.user); // إرسال التحديث للسيرفر
  ...
  const handleGetSkillsUser = async () => {
      ...
      setUserRateSkills(userData.Skills.map((skill) => skill.rating || 1)); // تحديث التقييمات
