...
import { EvilIcons,AntDesign,Feather,FontAwesome5,FontAwesome,MaterialIcons
...
setDropdownVisible(!isDropdownVisible);
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      ...
      try {
        ...
        if(Images){
         console.log(Images);
                 if (Platform.OS === 'web') {
                   // استخدام Blob للويب
                   const profileBlob = base64ToBlob(Images, 'image/jpeg');
        ...
        if (Videos) {
          console.log("Uploading video:", Videos);
        
          if (Platform.OS === 'web') {
            // استخدام Blob للويب
            const videoBlob = base64ToBlob(Videos, 'video/mp4');  // تأكد من نوع الفيديو الصحيح
            formData.append('videos', videoBlob, 'video.mp4');
          } else {
            console.log("Uploading video on mobile:", Videos);
        
            // استخدام uri للموبايل
            formData.append('videos', {
              uri: Videos, // تصحيح uri لاستخدام فيديو وليس صورة
              type: 'video/mp4', // تحديد النوع الصحيح
              name: 'video.mp4',
            });
          }
        }
        ...
        console.log('Add Post successfully',result.Post);
    ...
    const [selectedMedia, setSelectedMedia] = useState(null); // وسائط المختارة
    const pickMedia = async (type) => {
      setModalVisiblepost(false); // إغلاق القائم   
      ...
      try {
        ...
        if (!result.canceled) {
          if(type === "image"){
            setImages(result.assets[0].uri ); // تخزين الرابط الكامل والاسم
            setSelectedMedia({ uri: result.assets[0].uri, type });
  
          }
          if(type === "video")
          setVideos(result.assets[0].uri ); // تخزين الرابط الكامل والاسم
          setSelectedMedia({ uri: result.assets[0].uri, type });
    ...
    const handlePost = () => {
        if (text.trim() || media) {
            console.log("Posting:", { text, media });
            setText('');
            setMedia(null);
        }
    };
    
    const [postText, setPostText] = useState(""); // نص المنشور
    const [selectedImage, setSelectedImage] = useState(null); // الصورة المختارة
  
    const canPost = postText.trim() !== "" || selectedImage !== null;
    const [modalVisiblepost, setModalVisiblepost] = useState(false); // عرض القائمة المنبثقة
 // فتح قائمة اختيار الوسائط
 const openMediaOptions = () => {
  setModalVisiblepost(true);
};
const [selectedSkill, setSelectedSkill] = useState(""); // المهارة المختارة
const [skills, setSkills] = useState([]); // المهارة المختارة
const handleGetSkills = async () => {
 ...
 useEffect(() => {
  ...
  <ScrollView contentContainerStyle={{ padding: 10, backgroundColor: isNightMode ? "#000" : "#fff" }}>
  <View style={{ flex: 1, backgroundColor: "#fff", padding: 10 }}>
      {/* زر إضافة منشور */}
      <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 15 }}>
        <TouchableOpacity onPress={openMediaOptions}>
          <Text
            style={{
              fontSize: 16,
              fontWeight: "bold",
        ...
        <TouchableOpacity>
          <AntDesign name="caretdown" size={16} color="#000" style={{ marginLeft: 5 }} />
        </TouchableOpacity>
      </View>
  ...
  <View
      ...
      <Picker
        ...
        >
          <Picker.Item label="Select Feild" value="" />
          {Feilds.map((skill) => (
            <Picker.Item key={skill.id} label={skill.sub_specialization} value={skill.sub_specialization} />
          ))}
        </Picker>
      </View>

      {/* عرض الوسائط المختارة قبل النشر */}
      {selectedMedia && (
        <View style={{ marginTop: 10 }}>
          ...
          {selectedMedia.type === "video" && (
            <View style={{ alignItems: "center", marginVertical: 10 }}>
              <Ionicons name="videocam" size={50} color="red" />
              <Text style={{ marginTop: 5, fontSize: 16, color: "#555" }}>Video Selected</Text>
            </View>
          )}
          {selectedMedia.type === "file" && (
            <View style={{ alignItems: "center", marginVertical: 10 }}>
              <MaterialIcons name="insert-drive-file" size={50} color="blue" />
              <Text style={{ marginTop: 5, fontSize: 16, color: "#555" }}>{selectedMedia.name}</Text>
            </View>
          )}
        </View>
      )}
      ...
      <TouchableOpacity
        ...
        disabled={!canPost}
        style={{
          backgroundColor: canPost ? Colors.darkLight : "#ccc",
          padding: 10,
          borderRadius: 10,
          marginTop: 10,
          alignItems: "center",
      ...
      {/* قائمة الخيارات لاختيار الوسائط */}
      <Modal visible={modalVisiblepost} transparent animationType="slide">
        <View
          style={{
            flex: 1,
        ...
        >
          ...
          >
            <Text style={{ fontSize: 18, fontWeight: "bold", marginBottom: 10 }}>Choose Media</Text>
            ...
            >
              <FontAwesome name="image" size={24} color="green" style={{ marginRight: 10 }} />
            ...
            <TouchableOpacity
              ...
              style={{ flexDirection: "row", alignItems: "center", paddingVertical: 10 }}
            >
              <Ionicons name="videocam" size={24} color="red" style={{ marginRight: 10 }} />
              <Text style={{ fontSize: 16 }}>Select Video</Text>
            </TouchableOpacity>
            ...
            {/* زر إلغاء */}
            <TouchableOpacity onPress={() => setModalVisiblepost(false)}>
              <Text
                style={{
                  textAlign: "center",
                  color: "red",
