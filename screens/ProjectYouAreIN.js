...
import jwtDecode from 'jwt-decode'; // استيراد مكت
    ...
    const applyFilters = () => {
     ...
     //       console.log('Fetched Feilds:', data.Fields); // تحقق من البيانات
        ...
        const viewOwnProjectCreated = async () => {
            setIsSenior(false); // التهيئة الافتراضية
            ...
            try {
              ...
              // فك تشفير التوكن لاستخراج الدور
      ...
      setUserRoleee(userRole);
              ...
              const response = await fetch(apiUrl, {
                method: 'GET',
                headers: {
                  'Authorization': `Wasan__${token}`, // إضافة التوكن إلى الهيدر
                },
              });
              ...
              if (data.projects && data.projects.length > 0) {
                setProject(data.projects); // تعيين المشاريع في الحالة
                console.log('Fetched Projects:', data.projects); // تحقق من البيانات
              } else {
                console.log('No projects found'); // إذا لم يتم العثور على مشاريع
              }
          
            } catch (error) {
              console.error('Error fetching projects:', error.message); // معالجة الخطأ
            }
          };
